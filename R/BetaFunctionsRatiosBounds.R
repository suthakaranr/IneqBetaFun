...
them_3_9 = function(x1,x2){
  if(x1 > 0 && x2 >0 ){
    if( x1 <= x2 ){
      return(c("boud_a =", beta(x2,x2)/beta(x1,x1), "upper bound = ", (1/4)^(x2-x1)  ))
    }
    else ( x1 > x2 )
    return(c("boud_b =", beta(x2,x2)/beta(x1,x1), "lower bound = ", (1/4)^(x2-x1)  ))
  }
  else{
...
them_3_12 = function(y, x1,x2){
  if(y > 0 && x1 > 0 && x1 <= x2 && x2 < Inf ){
    return(c("beta_a =", beta(x2,y)/beta(x1,y),
...
corollary_ratio_1 = function(x1,x2, y1, y2){
  ...
  if( y1 > 0 && y1 <= y2 && y2 < Inf  && x1 > 0 && x1 <= x2 && x2 < Inf ){
    return(c("beta_xy =", beta(x2,y2)/beta(x1,y1), 
...
them_3_13 = function(y, x1,x2){
  if(y > 0 && x1 > 0 && x1 <= x2 && x2 < Inf ){
    return(c("beta_a =", beta(x2,y)/beta(x1,y), "upper bound = ", ((x1+y)/(x2+y))^y  ))
  }
  else{
